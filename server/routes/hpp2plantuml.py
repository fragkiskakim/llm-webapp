...
class Diagram(object):
    ...
    def parse_objects(self, header_file, arg_type='string'):
        ...
        for name, cls in parsed_header.classes.items():
            print(f"  class '{name}' → namespace='{cls.get('namespace','')}' parent='{cls.get('parent','')}'")
    ...
    def find_parent(self, parent_in, obj_ns_list_base, f_cmp=None):
        ...
        parent = parent_s[-1]
        explicit_ns = parent_s[:-1]  # e.g. ['Model'] from 'Model::DatabaseManager'

        # Αν υπάρχει explicit namespace prefix, δοκίμασε πρώτα direct match
        if explicit_ns:
            for c, c_n, c_ns in zip(class_list_obj, class_list, class_list_ns):
                obj_other_ns = c_ns.split('::')[:-1]
                if obj_other_ns == explicit_ns and (f_cmp(parent, c_n) or
                                                    f_cmp(parent, c_ns)):
                    return c['obj']

        # Fallback: original αλγόριθμος με obj_ns_list_base
        obj_ns_list = obj_ns_list_base + explicit_ns
        pi = 0
    ...
    def build_dependency_list(self):
        ...
        # ← ΠΡΟΣΘΗΚΗ: αφαίρεση διπλότυπων
        seen = set()
        unique_deps = []
        for dep in self._dependency_list:
            key = (dep._parent, dep._child)
            if key not in seen:
                seen.add(key)
                unique_deps.append(dep)
        self._dependency_list = unique_deps
...
def _extract_base_type(type_str):
    """Extract base type name for relationship lookup
    
    Strips qualifiers (const, &, *, []) and container wrappers
    (vector, unique_ptr etc.) to get the bare type name.
    """
    # Remove const, &, *, []
    t = re.sub(r'\bconst\b', '', type_str)
    t = re.sub(r'[&*\[\]]', '', t)
    t = t.strip()
    # Unwrap common containers: vector<X>, unique_ptr<X>, etc.
    m = re.search(r'\w+<([^>]+)>', t)
    if m:
        t = m.group(1).strip()
    return t
