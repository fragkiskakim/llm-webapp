...
const PROJECT = "mycharts"; // "dcc" ή "mycharts"

const ARCHITECTURES = ["3tier", "microservices"];
...
async function runOne(architecture, model, promptType, temperature, attempt) {
    console.log(`\n▶ [${PROJECT}] [${architecture}] [${model}] [${promptType}] [temp=${temperature}] attempt ${attempt}...`);
...
async function main() {
    ...
    console.log(`\n🚀 Τρέχω ${total} experiments συνολικά για project=${PROJECT}...`);
    ...
    if (failure > 0) {
        ...
        results
            .filter(r => !r.ok)
            .forEach(r =>
                console.log(`  - [${r.project}] [${r.architecture}] [${r.model}] [${r.promptType}] [temp=${r.temperature}] attempt ${r.attempt}: ${r.error}`)
            );
    }
    ...
    const outPath = require("path").join(
        __dirname,
        `../logs/${PROJECT}_batch_${Date.now()}.json`
    );
